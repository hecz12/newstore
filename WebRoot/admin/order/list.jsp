									...
									<c:forEach items="${list }" var="c" varStatus="vs">
										<tr onmouseover="this.style.backgroundColor = 'white'"
											...
											<td style="CURSOR: hand; HEIGHT: 22px" align="center"
												...
												<c:if test="${c.state==1 }">
													<c:if test="${c.state==1 }"><a href='${pageContext.request.contextPath }/adminOrderServlet?method=updateState&oid=${c.oid}&state=2'>发货</a></c:if>
												</c:if>
												<c:if test="${c.state==2 }">
													等待确认收货
												</c:if>
											...
											<td align="center" style="HEIGHT: 22px">
												<input type="button" value="订单详情"  onclick="showDetail('${c.oid}')"/>
											</td>
